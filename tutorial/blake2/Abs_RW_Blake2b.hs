...
{-# INLINE stageI #-}
-- stageI :: M () -> Dev (Inp (W 64, W 64 , W 64 , W 64))
stageI :: Monad m => m a -> ReacT i (Maybe o) m i
stageI x        = do
                   lift x
                   i' <- signal Nothing
                   return i' 
...
{-# INLINE stageU #-}
-- stageU :: M () -> Dev ()
stageU :: Monad m => m a -> ReacT i (Maybe o) m ()
stageU x        = do
                   lift x
                   signal Nothing
                   return ()
...
-- crypto_mixing = do
--                    stageI $ do message_permutation sigma0
--                               message_permutation sigma1
--                    stageI $ do message_permutation sigma2
--                               message_permutation sigma3
--                    stageI $ do message_permutation sigma4
--                               message_permutation sigma5
--                    stageI $ do message_permutation sigma6
--                               message_permutation sigma7
--                    stageI $ do message_permutation sigma8
--                               message_permutation sigma9
--                    stageI $ do message_permutation sigma0
--                               message_permutation sigma1
...
_F t f = do
           stageI $ do
             ...
             if f then
                    ...
                    return ()
           stageU crypto_mixing
           stageU xor_two_halves
...
abs_blake2b :: Monad m =>
               ...
               (i -> m a2)                        -> -- x2
               (i -> m a3)                        -> -- x3
               (i -> ReacT (Inp i) (Maybe o) m o) -> -- k
               Inp i                              ->
               ReacT (Inp i) (Maybe o) m ()
abs_blake2b x0 x1 x2 x3 k (Q0 arg1) = do
     i <- stageI (x0 arg1)
     abs_blake2b x0 x1 x2 x3 k i
abs_blake2b x0 x1 x2 x3 k (Q1 arg2) = do
     i <- stageI (x1 arg2)
     abs_blake2b x0 x1 x2 x3 k i
abs_blake2b x0 x1 x2 x3 k (Q2 arg3) = do
     i <- stageI (x2 arg3)
     abs_blake2b x0 x1 x2 x3 k i
abs_blake2b x0 x1 x2 x3 k (Q3 arg4) = do
     i <- stageI (x3 arg4)
     abs_blake2b x0 x1 x2 x3 k i
abs_blake2b x0 x1 x2 x3 k (Args args) = do
     v <- k args
     i <- signal (Just v)
     abs_blake2b x0 x1 x2 x3 k i

-- |
-- | My Putative Theorem: blake2b == twin_blake2b.
-- | This could be even further abstracted because _BLAKE2b is also staged.
-- |
-- | The main point of this is that the correctness of construction of rewire_blake2b from ref_blake2b
-- | doesn't depend on the individual actions (e.g., x1, x2, etc.) themselves.
-- |
twin_blake2b :: Inp (W 64, W 64 , W 64 , W 64) -> Dev ()
twin_blake2b = abs_blake2b x0 x1 x2 x3 k
  where
    x0 (w0 , w1 , w2 , w3)     = do
                                   setReg M0 w0
                                   setReg M1 w1
    ...
    x2 (w8 , w9 , w10 , w11)   = do
                                   ...
                                   setReg M10 w10
                                   setReg M11 w11
    x3 (w12 , w13 , w14 , w15) = do
                                   setReg M12 w12
                                   setReg M13 w13
                                   setReg M14 w14
                                   setReg M15 w15
    k :: (W 64, W 64 , W 64 , W 64) -> Dev W64x8
    k (ll_hi , ll_lo , kk , nn) = _BLAKE2b (ll_hi ++ ll_lo) kk nn
