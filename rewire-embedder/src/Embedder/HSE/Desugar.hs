...
desugar rn = flip evalStateT 0 .
      ...
      >=> pass
            ( depatLambdas   -- (opt) some pats allowed in lambdas, others not.
           <> lambdasToCases -- (opt) this could be good, but the goal is to generate fewer beta redexes anyway
            )
      -- >=> pass flattenAlts -- 1. Moved this to Atmo
      >=> pass desugarGuards
      >=> pass
      ...
      >=> pass
            ( desugarLets -- (opt) (4.) Move this to Atmo
           <> desugarNegLitPats
           -- <> desugarTuples -- 0. Moved this to Atmo
           <> normIds
           <> deparenify
           <> normTyContext
           <> desugarTyFuns
      ...
      >=> pass
              desugarAsPats
           -- <> liftDiscriminator -- 2. Moved this to Atmo
...
-- TODO: Check the TyFun dependency... seems irrelevant?
