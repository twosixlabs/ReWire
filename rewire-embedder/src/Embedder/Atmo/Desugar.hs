...
import Embedder.Atmo.Syntax (Exp(..),Module(..),Pat(..),Ty(..),
      DataCon(..),DataHeader,Binds(..),PatBind(..),FunBinding(..),
...
-- TODO:
-- Comment out all passes
-- Make Traversal that typechecks and seems right
...
data Desugar m = Desugar
      ...
      , dsPat      :: Tr m Pat
      , dsConDecl  :: Tr m DataCon
      , dsExp      :: Tr m Exp
      , dsType     :: Tr m Ty
      , dsDeclHead :: Tr m DataHeader
      , dsBinds    :: Tr m Binds
      , dsMatch    :: Tr m FunBinding
      , dsPatBind  :: Tr m PatBind
      , dsQName    :: Tr m Text
      , dsTySyn    :: Tr m TypeSynonym
      , dsDataDef  :: Tr m DataDefn
      , dsDefn     :: Tr m Defn
      }

instance Monad m => Semigroup (Desugar m) where
      (<>) (Desugar f1 f2 f3 f4 f5 f6 f7 f8 f9 f10 f11 f12 f13)
           (Desugar g1 g2 g3 g4 g5 g6 g7 g8 g9 g10 g11 g12 g13)
            = Desugar (f1  <> g1)
                      (f2  <> g2)
                      (f3  <> g3)
                      (f4  <> g4)
...
instance Monad m => Monoid (Desugar m) where
      mempty = Desugar TId TId TId TId TId TId TId TId TId TId TId TId TId

pass :: Monad m => Desugar m -> Module -> m Module
pass (Desugar f1 f2 f3 f4 f5 f6 f7 f8 f9 f10 f11 f12 f13)
      =   transformTr f13
      >=> transformTr f12
      >=> transformTr f11
      >=> transformTr f10
...
desugarAtmo rn = liftDesugar $ flip evalStateT 0 .
      ( pure
      -- >=> pass desugarInfix -- record info from this dusugaring pass for printing?
      >=> pass
      --       ( desugarNegs
      --      <> desugarDos
      --      <> desugarInfix
      ...
      --      <> desugarRecords rn
      --       )
      -- >=> pass flattenLambdas -- (opt) Move this to Atmo
      -- >=> pass
      --       ( depatLambdas
      --      <> lambdasToCases
      --       )
      -- >=> pass flattenAlts -- 1. Move this to Atmo
      -- >=> pass desugarGuards
      -- >=> pass
      --       ( desugarIfs -- (opt) Move this to Atmo
      --      <> wheresToLets
      --       )
         >=> pass
      --       ( desugarLets -- (opt) Move this to Atmo
      --      <> desugarNegLitPats
              desugarTuples
      --      <> normIds
      --      <> deparenify
...
-- type FieldInfo = (Text, (Text, Ty, Int, Int))
...
--                                                  [ PatBind (an "PatBind") (PApp (an "PApp") (UnQual (an "PApp") ctor) (argPats x' i arr))
...
--             pfToPat :: FQName -> PatField Annote -> Pat
--             pfToPat f = \ case
--                   PFieldPat _ _ p  -> p
...
--             fupToExp :: FQName -> FieldUpdate Annote -> Exp
--             fupToExp f = \ case
--                   FieldUpdate _ _ e  -> e
...
desugarFuns = mempty
      ...
      , data FunBinding = FunBinding Annote !Text ![Pat] !Exp
      ...
      where desugarFun :: (MonadState Fresh m, MonadError AstError m) => [(Name Annote, Type Annote)] -> Decl Annote -> m (Decl Annote)
            desugarFun ts = \ case
                  FunBind l ms@(Match l' name pats _ _:_) | allUnguarded ms -> 
                        pure $ FunBind l ms
            ...
            desugarPrimFunBinding :: (MonadState Fresh m, MonadError AstError m) => [FunBinding] -> m FunBinding
            desugarPrimFunBinding fbs = pure $ head fbs
            ...
            toPatBind ts = \ case
                  -- NOTE: can't type-annotate params without expanding type synonyms.
                  FunBinding l' [p] rhs -> pure $ PatBind (annotatePVars ts p) rhs
...
--             Case l1 v [PatBind l2 p (GuardedRhss l3 rhs) binds, PatBind l4 (PWildCard l5) (UnGuardedRhs l6 e') Nothing] -> do
--                   y <- fresh l1
--                   pure $ Case l1 e'
--                         [ PatBind l2 (PVar l2 y)
--                               ( UnGuardedRhs l3 $ Case l3 v
--                                     [ PatBind l3 p (UnGuardedRhs l3 $ toLet l3 (Var l6 $ UnQual l6 y) binds rhs) Nothing
--                                     , PatBind l4 (PWildCard l5) (UnGuardedRhs l6 $ Var l6 $ UnQual l6 y) Nothing
--                                     ]
--                               )
--                               Nothing
--                         ]
--             Case l1 v [PatBind l2 p (GuardedRhss l3 rhs) binds] ->
--                   pure $ Case l1 v [ PatBind l2 p (UnGuardedRhs l3 $ toLet l3 (err l1 "pattern match failure") binds rhs) Nothing ]
--             e -> pure e}
--       where toLet :: Annote -> Exp -> Maybe Binds -> [GuardedRhs Annote] -> Exp
--             toLet l y binds rhs = maybe id (Let l) binds $ foldr toIfs y rhs

--             toIfs :: GuardedRhs Annote -> Exp -> Exp
--             toIfs = \ case
--                   GuardedRhs l [Qualifier _ g1] e1 -> If l g1 e1
--                   _                                -> id

-- err :: Annote -> String -> Exp
...
--       e          -> pure e}
--       where transDo :: (MonadState Fresh m, MonadError AstError m) => Annote -> [Stmt Annote] -> m Exp
--             transDo l = \ case
...
--       Let _ (BDefs _ ds) e -> foldrM (transLet $ tySigMap ds) e $ filter isPatBind ds
--       n@Let{}               -> failAt (ann n) "Unsupported let syntax"
--       e                     -> pure e}
--       where transLet :: (MonadState Fresh m, MonadError AstError m) => [(Text, Ty)] -> Decl Annote -> Exp -> m Exp
--             transLet ts (PatBind l p (UnGuardedRhs l' e1) Nothing) inner = pure $ Case l e1 [PatBind l (annotatePVars ts p) (UnGuardedRhs l' inner) Nothing]
...
--             [ PatBind l (PApp l (UnQual l $ Ident l "True")  []) (UnGuardedRhs l e2) Nothing
--             , PatBind l (PApp l (UnQual l $ Ident l "False") []) (UnGuardedRhs l e3) Nothing
--             ]
--       e             -> e}
...
--       Lambda l [p] e            -> do
--             x <- fresh l
--             pure $ Lambda l [PVar l x] (Case l (Var l $ UnQual l x) [PatBind l p (UnGuardedRhs l e) Nothing])
--       e                         -> pure e}
...
-- desugarAsPats = mempty { dsPatBind = TM $ \ case
--       PatBind l p (UnGuardedRhs l' e) Nothing -> do
--             p'  <- deWild p
--             app <- foldrM (mkApp l) e $ getAses p'
--             pure $ PatBind l (deAs p') (UnGuardedRhs l' app) Nothing
--       e                                   -> pure e}

--       where mkApp :: (MonadState Fresh m, MonadError AstError m) => Annote -> (Pat, Pat) -> Exp -> m Exp
...
--             deWild :: MonadState Fresh m => Pat -> m Pat
--             deWild = transformM $ \ case
--                   PatWildCard l -> PVar l <$> fresh l
--                   p           -> pure p

--             patToExp :: (MonadState Fresh m, MonadError AstError m) => Pat -> m Exp
--             patToExp = \ case
--                   PatVar l n                -> pure $ Var l $ UnQual l n
...
--       App l (Lambda _ [p] e2) e1 -> Case l e1 [PatBind l p (UnGuardedRhs l e2) Nothing]
--       e                          -> e}


-- ========================================================================
-- Probably going to leave these alone... want to desugar these early
-- ========================================================================
...
{-Specifically, we need to desugar guards from PatBind, Match, and PatBind Decls -}
...
wheresToLets = mempty
      ...
      , dsPatBind = TM $ \ case
            PatBind l p (UnGuardedRhs l' e) (Just binds) -> pure $ PatBind l p (UnGuardedRhs l' $ Let l' binds e) Nothing
            a@(PatBind l _ (GuardedRhss _ _) _)          -> failAt (l :: Annote) $ "Guards are not supported: " <> pack (show $ void a)
            a                                        -> pure a
      }
-}

-- ========================================================================
-- Probably going to leave these alone... incl. structures we don't need in Atmo
-- ========================================================================

{-
-- TODO: Might be cool to record infix info and use it to help automate printing
-- BEFORE: desugarFuns
